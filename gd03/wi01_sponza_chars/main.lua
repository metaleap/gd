...
function Main()
  ...
  if emitter ~= INVALID_ENTITY then scene.Entity_Remove(emitter) end
  ...
  while true do
    ...
    camera_movement =
      vector.Rotate(camera_movement, cam_transform.Rotation_local) -- rotate the camera movement with camera orientation, so it's relative
